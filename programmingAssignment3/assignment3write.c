...
#define  DEVICE_NAME "grp42write"    ///< The device will appear at /dev/grp42 using this value
#define  CLASS_NAME  "grp42write"       
#define  BUFFERSIZE 1024
MODULE_LICENSE("GPL");            
MODULE_AUTHOR("Group 42 ");    
MODULE_DESCRIPTION("Assignment 2 Write Module");  
MODULE_VERSION("0.1");            
...
static int __init ebbchar_init(void){
   printk(KERN_INFO "group42Write: Initializing the group42 Writw LKM\n");

   // Try to dynamically allocate a major number for the device -- more difficult but worth it
   majorNumber = register_chrdev(0, DEVICE_NAME, &fops);
   if (majorNumber<0){
      printk(KERN_ALERT "group42Write failed to register a major number\n");
      return majorNumber;
   }
   ...
   printk(KERN_INFO "group42Write: device class registered correctly with major number :%d\n",majorNumber);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   printk(KERN_INFO "Group42: This is write-only module please use the groupd42Read module \n");
